...
import AdminCustDetMobile from "./AdminCustDetMobile";
...
const AdminCustomersDetails = () => {
  ...
  const filteredUsers = customers
    .filter(user => user.userType === 2) // Ensure only customers
   .filter((user) => {
  const searchIn = `${user.userId ?? ""} ${user.username ?? ""} ${user.email ?? ""} ${user.phoneNo ?? ""}`.toLowerCase();
  return searchIn.includes(searchQuery.toLowerCase());
});


  // Mock bills data linked to userId for demo
  const bills = [
  ...
  // Calculate total spent and total orders per user
  const calculateTotalSpentAndOrders = (userId) => {
    const userBills = bills.filter((bill) => bill.userId === userId);
    const totalSpent = userBills.reduce(
      (acc, bill) => acc + parseFloat(bill.billPrice),
      0
    );
        ...
        <>
          <div className="orders-list">
            <table>
              ...
              <tbody>
                {paginateOrders(filteredUsers, currentPage, ordersPerPage).map(
                  (user, index) => {
                    const { totalSpent, totalOrders } = calculateTotalSpentAndOrders(
                      user._id
                    );
                    return (
                      <React.Fragment key={user._id}>
                        <tr
                          ...
                          className={`order-row ${
                            activeOrder === user._id ? "active-order" : ""
                          }`}
                        >
                        ...
                        {activeOrder === user._id && (
                          <tr
                            className={`order-details-row ${
                              activeOrder === user._id ? "active-order-details" : ""
                            }`}
                          >
                            <td colSpan="6">
